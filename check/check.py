...
def check_parse():
    ...
    with open(rf'{apath}check/orderfull.txt', 'r', encoding='utf8') as file:  # считываем информацию про аккаунты
    ...
    df.to_excel(f"{apath}/Статус аккаунтов {datetime.datetime.now().strftime('%H-%M-%S')}.xlsx", index=False)