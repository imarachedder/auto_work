...
def coverage_parse():
    with open('settings_barkov.txt', 'r', encoding='utf8') as file:  # считываем настройки
        set = file.read().split('\n')
    ...
    for mf in os.listdir("Готовые файлы"):
        with open(f"Готовые файлы/{mf}", encoding="utf-8") as f:
            ...
            urls = []
            with open(f"{name_excel}", encoding='utf-8') as r_file:  # считываем скаченный файл
    ...
    name_file = f"./просмотры-{datetime.date.today()}.csv"
    is_create_row = False
