...
def parseBarkov():
    with open(rf'settings_barkov.txt', 'r', encoding='utf8') as file:  # считываем настройки
        settings = file.read().split('\n')

    df = pd.read_excel('settings_barkov.xlsx')
    ...
    for text in text_excel:
        ...
        urls = []
        with open(rf"{name_excel}", encoding='utf-8') as r_file:  # считываем скаченный файл
